 ...
 * @param {Object} _dir 
 */
const expiredCheck = (Badboy, _dir) => {
    setInterval(() => {
        let position = null
        Object.keys(_dir).forEach((i) => {
        ...
        if (position !== null) {
            ...
            if (_dir[position].status === true){
                Badboy.sendMessage(_dir[position].id, `Waktu sewa di grup ini sudah habis, bot akan keluar otomatis`, MessageType.text)
                .then(() => {
                    Badboy.groupLeave(_dir[position].id)
                    .then(() => {
