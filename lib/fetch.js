...
exports.createExif = (pack, auth) =>{
    ...
    const exif = {"sticker-pack-id": "com.badboy.tech", "sticker-pack-name": pack, "sticker-pack-publisher": auth, "android-app-store-link": "https://play.google.com/store/apps/details?id=com.termux", "ios-app-store-link": "https://itunes.apple.com/app/sticker-maker-studio/id1443326857"}
...
exports.modStick = (media, badboy, mek, from) => {
    out = getRandom('.webp')
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './sticker/data.exif', media, '-o', out])
        .on('exit', () => {
            badboy.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: mek})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        badboy.sendMessage(from, 'There is an error', 'conversation', { quoted: mek })
        fs.unlinkSync(media)
    }
}
